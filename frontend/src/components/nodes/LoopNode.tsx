...
import { Repeat, Trash2, Play, Maximize2, Minimize2 } from 'lucide-react';
...
export function LoopNode({ id, data }: { id: string, data: any }) {
    ...
    return (
        <div
            ...
            className={`bg-slate-900 border-2 border-slate-700 hover:border-purple-500 rounded-xl shadow-xl transition-all group relative ${expanded ? 'pb-8 pr-4 min-w-[280px]' : 'min-w-[200px]'}`}
        >
            ...
            {/* Output Handles - Visible even when expanded? Or hide? Usually handles should stay. */}
            {/* But layout changes when collapsed. 
                When collapsed (expanded=false), we probably just want standard handles on simple box.
                Relative positioning relies on pb-8 pr-4 padding which is removed.
            */}

            {/* 'Do' Handle (Right) */}
            <div className={`absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 flex items-center ${!expanded && 'translate-x-[4px]'}`}>
                {expanded && (
            ...
            {/* 'Done' Handle (Bottom) */}
            <div className={`absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 flex flex-col items-center ${!expanded && 'translate-y-[4px]'}`}>
                <Handle
                    type="source"

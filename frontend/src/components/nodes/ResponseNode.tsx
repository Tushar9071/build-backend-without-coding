...
export function ResponseNode({ id, data }: { id: string, data: any }) {
    ...
    const getStatusColor = (code: number) => {
        if (code >= 200 && code < 300) return 'text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-500/20 border-green-200 dark:border-green-500/50';
        if (code >= 400 && code < 500) return 'text-orange-600 dark:text-orange-400 bg-orange-100 dark:bg-orange-500/20 border-orange-200 dark:border-orange-500/50';
        if (code >= 500) return 'text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-500/20 border-red-200 dark:border-red-500/50';
        return 'text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-500/20 border-slate-200 dark:border-slate-500/50';
    };
    ...
    return (
        <div
            ...
            className={`bg-white dark:bg-slate-900 border-2 ${getHeaderColor()} rounded-xl shadow-xl transition-all group ${expanded ? 'min-w-[280px]' : 'min-w-[200px]'}`}
        >
            ...
            <Handle
                ...
                className="!bg-white !w-3 !h-3 !border-2 !border-slate-200 dark:!border-slate-900"
            />
            ...
            <div className="bg-slate-50 dark:bg-slate-950 p-3 rounded-t-xl border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <div className="p-1.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-200 rounded-lg">
                        <Send className="w-4 h-4" />
                    </div>
                    <span className="font-semibold text-slate-900 dark:text-white text-sm">Return Response</span>
                </div>
            ...
            {expanded && (
                <div className="p-4 space-y-3">
                    ...
                    {/* Response Body Type */}
                    <div className="flex bg-slate-100 dark:bg-slate-950 p-1 rounded-lg border border-slate-200 dark:border-slate-800 mt-2">
                        {['json', 'variable'].map((t) => (
                            <button
                                key={t}
                        ...
                        {responseType === 'json' ? (
                             ...
                             */}
                                <SuggestionInput
                                    ...
                                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2 text-xs font-mono text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 transition-colors mb-2"
                            ...
                            <div className="flex flex-col gap-2">
                                <div className="flex items-center bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2">
                                    ...
                                    <input
                                        ...
                                        className="flex-1 bg-transparent border-none text-xs text-slate-900 dark:text-white focus:outline-none font-mono"
                                    />
                                </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-1">
                                        {variables.map(v => (
                                            <button
                                                ...
                                                className={`px-2 py-1.5 rounded text-[10px] font-mono text-left truncate transition-colors flex items-center gap-2 ${body === v.name ? 'bg-cyan-100 dark:bg-cyan-500/20 text-cyan-600 dark:text-cyan-400 border border-cyan-200 dark:border-cyan-500/50' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'}`}
                                            >

...
async def update_workflow(workflow_id: str, workflow_update: WorkflowBase, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Workflow).filter(Workflow.id == workflow_id))
    workflow = result.scalars().first()
    if not workflow:
        raise HTTPException(status_code=404, detail="Workflow not found")
...
async def run_workflow(workflow_id: str, input_data: dict, db: AsyncSession = Depends(get_db)):
